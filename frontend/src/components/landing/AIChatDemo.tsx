  ...
  return (
    <section ref={ref} className="py-[var(--space-section)] px-6 bg-[#fbfbfd] dark:bg-black">
      <div className="max-w-[800px] mx-auto">
        ...
        {/* Chat container */}
        <div
          className={`rounded-[24px] border border-black/[0.08] dark:border-white/[0.06] bg-white dark:bg-[#1c1c1e] overflow-hidden shadow-[0_0_80px_-20px_rgba(10,132,255,0.12)] dark:shadow-[0_0_80px_-20px_rgba(10,132,255,0.1)] transition-all duration-700 ease-out delay-200 ${
            visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
          }`}
        >
          {/* Chat header */}
          <div className="flex items-center gap-3 px-6 py-4 border-b border-black/[0.06] dark:border-white/[0.04]">
            ...
            <div>
              <p className="text-[14px] font-medium text-[#1d1d1f] dark:text-white">Supply Chain Assistant</p>
          ...
          <div className="p-6 space-y-5 min-h-[320px]">
            ...
            <div className="flex gap-3">
              <div className="w-7 h-7 rounded-full bg-[#f5f5f7] dark:bg-white/[0.06] flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-[10px] font-bold text-[#0a84ff]">AI</span>
              </div>
              <div className="max-w-[82%] space-y-3">
                {/* Typing text */}
                <div className="px-5 py-3 rounded-[20px] rounded-tl-sm bg-[#f5f5f7] dark:bg-white/[0.04] text-[14px] text-[#1d1d1f] dark:text-white/80 leading-relaxed">
                  {typedText}
                  {typedText.length < messages[1].text.length && (
                    <span className="inline-block w-[1.5px] h-[14px] bg-black/60 dark:bg-white/60 ml-0.5 animate-pulse" />
                  )}
                </div>
                ...
                {showItems && messages[1].items && (
                  <div className="space-y-2 animate-fade-in-up">
                    {messages[1].items.map((item) => (
                      <div
                        key={item.name}
                        className="px-5 py-4 rounded-[16px] bg-[#f5f5f7] dark:bg-white/[0.03] border border-black/[0.06] dark:border-white/[0.06]"
                      >
                        ...
                        <p className="text-[12px] text-[#86868b] dark:text-white/30">{item.reason}</p>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {/* Input bar */}
          <div className="px-6 py-4 border-t border-black/[0.06] dark:border-white/[0.04]">
            <div className="flex items-center gap-3 px-4 py-3 rounded-full bg-[#f5f5f7] dark:bg-white/[0.04] border border-black/[0.06] dark:border-white/[0.06]">
              ...
              <span className="text-[13px] text-[#86868b] dark:text-white/20 flex-1">
                Ask anything about your supply chain...
              </span>
              <div className="w-7 h-7 rounded-full bg-[#1d1d1f] dark:bg-white flex items-center justify-center">
                <span className="text-white dark:text-black text-[12px] font-medium">→</span>
              </div>
            </div>
          </div>
        </div>
