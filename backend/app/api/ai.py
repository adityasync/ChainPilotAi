...
from ..core.exceptions import UnauthorizedError, NotFoundError, RateLimitError, ServiceUnavailableError
...
def stream_query(
    ...
    company_id = get_current_user_company_id(db, email)
    ...
    context = build_dashboard_context(db, company_id)
...
def generate_insights(
    ...
    context = build_insights_context(db, company_id)
    try:
        insights = generate_ai_insights(client, db, company_id, context)
    except GLMClientError as e:
...
def get_supplier_narrative(
    ...
    context = build_supplier_context(db, supplier_id, company_id)
    if not context:
        raise NotFoundError("Supplier")

    try:
        narrative_data = get_or_generate_narrative(client, db, supplier_id, company_id, context)
    except GLMClientError as e:
        from ..core.exceptions import AppError
        raise AppError(code="AI_SERVICE_ERROR", message=e.detail, status_code=502)
